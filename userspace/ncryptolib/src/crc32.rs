//! CRC32 Checksum (ISO 3309)
//!
//! Fast non-cryptographic checksum for file integrity verification.
//! CRC32 is widely used in:
//! - ZIP/GZIP archives
//! - PNG images
//! - Ethernet frames
//! - Many file formats
...
impl Crc32 {
    ...
    /// Get CRC32 as bytes (big-endian)
    pub fn finalize_bytes(&self) -> [u8; CRC32_SIZE] {
        self.finalize().to_be_bytes()
    }
}

/// Compute CRC32 checksum (convenience function)
...
/// Compute CRC32 checksum as bytes
pub fn crc32_bytes(data: &[u8]) -> [u8; CRC32_SIZE] {
    let mut hasher = Crc32::new();
    hasher.update(data);
...
// ============================================================================
// CRC32C (Castagnoli) - Used by iSCSI, SCTP, ext4
// ============================================================================
...
/// CRC32C hasher state (Castagnoli polynomial)
#[derive(Clone)]
pub struct Crc32c {
    state: u32,
}
...
#[no_mangle]
pub extern "C" fn CRC32(crc: u32, data: *const u8, len: usize) -> u32 {
...
/// Initialize CRC32 (returns initial value)
#[no_mangle]
pub extern "C" fn crc32_init() -> u32 {
    0
}

/// Compute CRC32C checksum
#[no_mangle]
pub extern "C" fn CRC32C(crc: u32, data: *const u8, len: usize) -> u32 {
    if data.is_null() && len > 0 {
        return crc;
    }
...
mod tests {
    ...
    #[test]
    fn test_crc32_hello() {
        // "Hello, World!" should produce a consistent CRC
        let crc = crc32(b"Hello, World!");
        assert_ne!(crc, 0);
    }

    #[test]
    fn test_crc32c_123456789() {
        // CRC32C test vector
