...
use crate::kex::{
    derive_tls12_keys, derive_tls13_application_keys, derive_tls13_handshake_keys,
    derive_tls13_keys, derive_tls13_traffic_secret, hkdf_expand_label, hkdf_expand_label_sha384,
    HandshakeSecrets, KeyExchange,
};
use crate::record::RecordLayer;
use crate::session::SslSession;
use crate::tls::{ContentType, ExtensionType, HandshakeType, NamedGroup};
use crate::x509::{X509Stack, X509};
use crate::{c_char, c_int, c_uchar, TLS1_2_VERSION, TLS1_3_VERSION};
...
impl SslConnection {
    ...
    fn send_server_hello(&mut self) -> bool {
        ...
        if let Some(c) = cipher {
            ...
            let hello =
                self.handshake
                    .build_server_hello(self.version, cipher_id, &self.alpn_protos);
            self.record.write_handshake(&hello, self.wbio)
        } else {
            false
        }
    ...
    fn do_tls13_client_handshake(&mut self) -> c_int {
        ...
        let (hs_secrets, hs_keys) =
            match derive_tls13_handshake_keys(&shared_secret, &transcript_hash, key_len) {
                Some(k) => k,
                None => {
                    self.last_error = crate::ssl_error::SSL_ERROR_SSL;
                    return -1;
        ...
        if !self.receive_tls13_finished_with_secret(
            &hs_secrets.server_hs_traffic_secret,
        ...
        if !self.send_tls13_finished_with_secret(
            &hs_secrets.client_hs_traffic_secret,
        ...
        let app_keys = match derive_tls13_application_keys(
            &hs_secrets.handshake_secret,
            &client_finished_hash,
            key_len,
        ) {
            Some(k) => k,
    ...
    fn receive_tls13_finished(
        ...
        let traffic_secret =
            derive_tls13_traffic_secret(handshake_secret, label, transcript_hash, use_sha384);
    ...
    fn receive_tls13_finished_with_secret(
        &mut self,
        traffic_secret: &[u8],
        transcript_hash: &[u8],
    ) -> bool {
    ...
    fn send_tls13_finished_with_secret(
        &mut self,
        traffic_secret: &[u8],
        transcript_hash: &[u8],
    ) -> bool {
    ...
    fn select_cipher(&self) -> Option<SslCipher> {
        ...
        if self.version >= TLS1_3_VERSION {
            self.ciphersuites
                .first()
                .and_then(|&id| SslCipher::from_id(id))
        } else {
            self.cipher_list
                .first()
                .and_then(|&id| SslCipher::from_id(id))
