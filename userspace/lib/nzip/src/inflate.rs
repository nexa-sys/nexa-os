...
impl Inflater {
    ...
    fn setup_fixed_huffman(&mut self) -> ZlibResult<()> {
        self.litlen_table
            .build(&FIXED_LITLEN_LENGTHS, LITLEN_CODES)
            .map_err(|_| ZlibError::DataError)?;
        self.dist_table
            .build(&FIXED_DIST_LENGTHS, DIST_CODES)
            .map_err(|_| ZlibError::DataError)?;
        Ok(())
    }
    ...
    fn read_dynamic_huffman(&mut self, input: &[u8], pos: &mut usize) -> ZlibResult<()> {
        ...
        codelen_table
            .build(&codelen_lengths, CODELEN_CODES)
            .map_err(|_| ZlibError::DataError)?;
        ...
        self.litlen_table
            .build(&lengths[..hlit], hlit)
            .map_err(|_| ZlibError::DataError)?;
        self.dist_table
            .build(&lengths[hlit..], hdist)
            .map_err(|_| ZlibError::DataError)?;
