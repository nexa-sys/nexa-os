...
/// Environment variable storage
static mut ENV_STORAGE: [[u8; MAX_ENV_ENTRY_LEN]; MAX_ENV_VARS] = [[0; MAX_ENV_ENTRY_LEN]; MAX_ENV_VARS];
...
pub unsafe extern "C" fn setenv(name: *const c_char, value: *const c_char, overwrite: c_int) -> c_int {
    ...
    let value_len = if value.is_null() { 0 } else { crate::strlen(value as *const u8) };
