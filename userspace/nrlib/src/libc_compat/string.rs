...
pub unsafe extern "C" fn strerror(errnum: c_int) -> *mut c_char {
    ...
    let offset = if errnum < 0 { prefix_len + 1 } else { prefix_len };
...
#[no_mangle]
pub unsafe extern "C" fn __xpg_strerror_r(errnum: c_int, buf: *mut c_char, buflen: size_t) -> c_int {
...
pub extern "C" fn isspace(c: c_int) -> c_int {
    let c = c as u8;
    if c == b' ' || c == b'\t' || c == b'\n' || c == b'\r' || c == b'\x0b' || c == b'\x0c' { 1 } else { 0 }
}

#[no_mangle]
pub extern "C" fn isupper(c: c_int) -> c_int {
    let c = c as u8;
    if c >= b'A' && c <= b'Z' { 1 } else { 0 }
}
...
pub extern "C" fn isxdigit(c: c_int) -> c_int {
    let c = c as u8;
    if (c >= b'0' && c <= b'9') || (c >= b'a' && c <= b'f') || (c >= b'A' && c <= b'F') { 1 } else { 0 }
}
