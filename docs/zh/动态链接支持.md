...
❌ **辅助向量**: 动态链接器需要通过辅助向量获取原始程序的信息（AT_PHDR、AT_ENTRY、AT_PHNUM 等）
❌ **程序加载**: 当前只加载解释器；原始程序也需要加载到单独的内存区域
❌ **共享库**: 不支持加载共享库（`.so` 文件）
...
实现堆栈设置以将辅助向量传递给动态链接器：

```rust
// 在 process.rs 中，跳转到用户模式之前
fn setup_auxv_stack(stack: &mut [u64], program_info: &ProgramInfo) {
    // 将辅助向量推送到堆栈
    // 格式：[type, value, type, value, ..., AT_NULL, 0]
}
```
...
要完全支持动态链接的程序，必须实现辅助向量以将程序信息传递给动态链接器。这是阻止动态程序正确执行的关键缺失部分。
