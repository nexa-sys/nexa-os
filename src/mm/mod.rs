...
pub mod numa {
    ...
    pub fn online_nodes() -> &'static [u32] {
        &[0]
    }
    pub fn get_node(_id: u32) -> Option<&'static NumaNode> {
        None
    }
    pub fn cpu_to_node(_cpu: u32) -> u32 {
        0
    }
    pub fn cpus_on_node(_node: u32) -> &'static [u32] {
        &[]
    }
    pub fn addr_to_node(_addr: u64) -> u32 {
        0
    }
    pub fn current_node() -> u32 {
        0
    }
    pub fn node_distance(_from: u32, _to: u32) -> u8 {
        LOCAL_DISTANCE
    }
    pub fn best_node_for_policy(_policy: NumaPolicy) -> u32 {
        0
    }
...
pub use paging::{
    activate_address_space, allocate_user_region, clear_user_mappings,
    create_process_address_space, current_pml4_phys, debug_cr3_info, ensure_nxe_enabled,
    free_process_address_space, free_user_region, handle_user_demand_fault, init,
    is_user_demand_page_address, kernel_pml4_phys, print_cr3_statistics,
    print_demand_paging_statistics, print_user_region_statistics, read_current_cr3, validate_cr3,
    MapDeviceError,
};
...
// Re-export from vma
pub use vma::{
    free_address_space, get_address_space, init_address_space, AddressSpace, VMABacking, VMAFlags,
    VMAManager, VMAPermissions, VMAStats, MAX_ADDRESS_SPACES, MAX_VMAS, PAGE_SIZE as VMA_PAGE_SIZE,
    VMA,
};
